...
export default {
  ...
  data() {
    return {
      ...
      moreParams: {
        query: `{vueTableRole{data {id  name description countFilteredUsers} total per_page current_page last_page prev_page_url next_page_url from to}}`
      }
    }
  },
  methods: {
    ...
    downloadExampleCsv: function() {
      var t = this
      axios.get(t.$baseUrl() + '/roles/example_csv', {
        responseType: 'blob'
      }).then((response) => {
