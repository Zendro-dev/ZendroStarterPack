<template>
  <div class="col-xs-5 content">
    <ul v-for="record in errors" v-if="errors" class="list-group">
      <li class="list-group-item">
        <div class="alert alert-danger">
          <h4>Errors for nuc_acid_library_result {{record.record}}</h4>
          <ul>
    ...
    <h4>Upload nuc_acid_library_results</h4>
      <form id="nuc_acid_library_result-form" enctype="multipart/form-data" novalidate v-on:submit.prevent="onSubmit">
  ...
  methods: {
    onSubmit() {
      ...
      if (t.$refs.uploadTable.value.indexOf('.xlsx') > 0) {
        ...
        query = `mutation {bulkAddNuc_acid_library_resultXlsx{ id }}`
      } else {
        var formElm = "csv_file"
        query = `mutation {bulkAddNuc_acid_library_resultCsv{ id}}`
      }
